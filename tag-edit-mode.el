...
`tag-edit-revert-all-file-tags'"
  ...
  (save-excursion
    (let* ((tag-edit-pulse-on-save nil)
           ...
           (files (mapcar (lambda (alist)
                            (cadr (assoc "file" alist)))
                          (hash-table-values tag-edit-files-original-tags)))
           (num-keys (length files))
           (tag-edit--current-backup-directory (tag-edit--backup-directory (f-common-parent files))))
      (dolist (index keys)
        (let ((index+1 (1+ index))
